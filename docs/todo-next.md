...
- Model editor preview canvas/window event binding now lives in `src/view/editors/model-editor-bindings.mjs`, with focused checks for pointer, wheel, resize, contextmenu, and `ModelEditor.initPreview` path guards.
- Model editor preview sizing and frame rendering now route through `src/view/editors/model-editor-pointer-ui.mjs`, with focused checks and `ModelEditor.handleResize`/`renderPreview` path guards.
...
- Floor supply drop chance planning now lives in `src/logic/floor-buffs.mjs`, with focused checks for chest/cache odds, buff bonuses, clamping, invalid input fallback, and controller path guards for normal and hidden-cache pickups.
...
- Combat dialogue typewriter DOM writes now live in `src/view/combat-dialogue-ui.mjs`, with focused checks for per-character delays, numeric coercion, null fallback, and the `GameController.typeText` path guard.
- Floating text DOM creation, mount/remove, and frame style application now live in `src/view/floating-text-ui.mjs`, with focused checks and `VisualEngine` path guards while `VisualEngine` keeps 3D projection and lifetime state.
...
- Theme-chain bonus visible message formatting now lives in `src/logic/room-reward-resolution.mjs`, with focused checks for ember, salvage, signal, quarantine, fallback labels, and the `GameController` path guard.
