...
- `CollectionPanel` and `PathDebugPanel` were extracted into `src/view/panels/` with explicit `GameState`/controller dependencies; PathDebugPanel routing summary counts now have focused checks.
...
- Model editor selection control/display-state calculation and DOM application were extracted into `src/view/editors/model-editor-selection-ui.mjs`, with focused checks and browser smoke coverage for idle disabled controls.
