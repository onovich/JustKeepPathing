...
- Keep hidden-room route scoring, small resolution predicates, reward calculations/plans, merchant purchase decisions, sound config sanitizing, and theme-chain formulas in `src/logic/` modules. (Started: `hidden-room-routing.mjs`, `hidden-room-resolution.mjs`, `room-reward-calculations.mjs`, `room-reward-plans.mjs`, `sound-config.mjs`, `theme-chain-bonuses.mjs`.)
